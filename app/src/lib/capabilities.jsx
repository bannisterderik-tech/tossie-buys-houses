 ...
 *
 * Loaded once per session. Roles change when an owner changes them, which is
 * rare and already requires the affected person to reload to see the difference
 * — polling for it would be a request per minute forever to catch an event that
...
export function CapabilitiesProvider({ children }) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const [{ data: role }, { data: rows }] = await Promise.all([
